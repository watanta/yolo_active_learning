...
class ActiveLearning:
    ...
    def select_next_sample(self, model, num_samples=100):
        ...
        # ラベルファイルが存在する画像のみをフィルタリング
        pool_images = []
        for img_path in (self.pool_dir / 'images').glob('*.jpg'):
            label_path = self.pool_dir / 'labels' / (img_path.stem + '.txt')
            if label_path.exists():
                pool_images.append(img_path)
        
        print(f"Found {len(pool_images)} images with labels in pool")
